...
| Credential transport | Native SecureStore; API-issued Web HttpOnly cookies; access token memory-only | Enforces SAFE-03 and keeps both refresh and pending-registration proofs outside Web JavaScript. Registration responses create/rotate the pending cookie; verification and logout responses clear the matching cookie attributes. |
...
- [ ] Database: a migrated PostgreSQL 18 schema with a real canonical-email read, account write, and hash-only pending-registration proof bound to the verification token.
...
- Production SMTP vendor/domain, public origins, app-store domains, and iOS association verification are explicit Phase 6 release gates with the release owner; local behavior is proven with Mailpit and configured local origins.
