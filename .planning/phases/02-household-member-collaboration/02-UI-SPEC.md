...
> 家庭组与成员协作阶段的视觉和交互契约。由 `gsd-ui-researcher` 生成，并经 `gsd-ui-checker` 六维验证通过。
...
| `space.2` | 8px | 紧凑元数据、徽标内间距、标签到字段及列表行内部次级间距 |
| `space.4` | 16px | 默认组件间距、列表行水平与垂直内边距、`<360px` 紧凑屏幕边距 |
...
- `AppShell` 使用安全区加 24px 水平边距；宽度小于 360px 时为 16px。
...
- 普通表单主按钮位于表单流末尾、全宽 52px；不永久固定在键盘上方。确认页的安全保留动作与确认动作保持可见顺序：安全保留动作在前，破坏性确认在后；安全保留动作必须使用下文为各流程指定的文案。
- 所有权转移候选使用单选列表；当前 owner 不可选；“继续转移所有权”按钮在选择有效继任者前禁用并声明 disabled 状态。owner 离开流程对应使用“继续离开家庭”。
...
| Range | Contract |
|-------|----------|
| `<360px` | 16px inset；标题和操作允许换行；不使用并排按钮 |
...
| 转移所有权 | 家庭、当前 owner、继任者单选、权限后果 | “继续转移所有权”后进入最终确认 |
| owner 离开家庭 | 继任者单选、所有权与离开原子完成说明 | “继续离开家庭”后进入最终确认 |
...
| 降级 admin 为 member | 独立确认页；显示成员、家庭、将失去的管理权限；安全动作“保留管理员权限”，确认动作“降级为成员” |
| 移除非 owner 成员 | 独立确认页；显示成员、家庭、立即失去访问权与内容不可见后果；安全动作“保留成员”，确认动作“移除成员” |
| 撤销邀请 | 独立确认页；显示邮箱、家庭、链接将失效；安全动作“保留邀请”，确认动作“撤销邀请” |
| 转移所有权 | 选择继任者页的动作“继续转移所有权” → 后果确认页 → `FinalConfirmation`；安全动作“保留当前所有者”，最终按钮“确认转移所有权” |
| owner 离开 | 选择继任者页的动作“继续离开家庭” → 后果确认页 → `FinalConfirmation`；安全动作“留在家庭”，最终按钮“转移所有权并离开” |

所有确认页默认焦点在标题；最终 dialog 默认焦点在该流程的安全动作（转移为“保留当前所有者”，owner 离开为“留在家庭”）。破坏性按钮和安全动作按钮不得颠倒视觉/DOM 顺序。操作 pending 后两个按钮均禁用；成功前不乐观移除成员或改变 owner 标识。
...
| Demote safe CTA | 保留管理员权限 |
| Remove confirmation | 从「{家庭名称}」移除 {成员名}？移除后，对方会立即失去这个家庭的访问权。 |
| Remove safe CTA | 保留成员 |
| Revoke confirmation | 撤销发给 {邮箱} 的邀请？撤销后，原链接将不能使用。 |
| Revoke safe CTA | 保留邀请 |
| Transfer continue CTA | 继续转移所有权 |
| Transfer safe CTA | 保留当前所有者 |
| Transfer final | 将「{家庭名称}」的所有权转移给 {成员名}？你将不再是所有者。 |
| Owner leave continue CTA | 继续离开家庭 |
| Owner leave safe CTA | 留在家庭 |
...
- 提交失败时 Web 焦点移动到错误摘要；字段错误关联到输入。确认页加载后焦点移动到标题，最终 dialog 默认焦点为该流程的“保留当前所有者”或“留在家庭”安全动作。
- 200% 文字缩放时家庭名称、邮箱、角色和后果说明完整可读；横屏 320px 等效宽度不得出现水平滚动。
- 减少动态效果时，弹层移除位移动画，仅保留 ≤80ms opacity；正常过渡沿用 180ms。forced-colors 下保留系统边框与选中标记。
...
| 分级确认 | 降级/移除一层确认；转移/owner 离开额外最终确认；逐流程断言“保留管理员权限 / 保留成员 / 保留邀请 / 保留当前所有者 / 留在家庭”无副作用 |
